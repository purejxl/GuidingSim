 ...
 */
public class runOnTimeBased implements Runnable{
    int numOfPeople;
    int rounds[];
    ...
    private void modifyEdgeWeight(Graph graph) {
        ArrayList<Edge> edges = graph.getRoadEdgeList();
        for (Edge edge : edges) {
            double density = edge.nPeople / edge.area;
            double duration = edge.length / GuidingSim.getPersonVelocity(density);
            graph.modifyWeightedEdge(edge, duration);
        }
    }
}
